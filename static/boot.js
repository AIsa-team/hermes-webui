...
function _normalizeAppearance(theme,skin){
  ...
  const nextTheme=legacy?legacy.theme:(_VALID_THEMES.has(rawTheme)?rawTheme:'system');
  const nextSkin=_VALID_SKINS.has(rawSkin)?rawSkin:(legacy?legacy.skin:'charizard');
...
function registerHermesSkin(descriptor){
  try{
    ...
    if(document.getElementById('skinPickerGrid')){
      _buildSkinPicker((localStorage.getItem('hermes-skin')||'charizard').toLowerCase());
    }
...
(async()=>{
  ...
  try{
    ...
    // localStorage into 'system'/'charizard' BEFORE this code runs, so a truly
    ...
    const lsHasExplicitSkin=lsSkin&&lsSkin!=='charizard';
